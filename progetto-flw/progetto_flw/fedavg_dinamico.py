...
class FedAvg(Strategy):
    ...
    def aggregate_fit(
        ...
        Path("logs_dinamico").mkdir(exist_ok=True)
        ...
        if server_round <= 2:
            ...
            for client, fit_res in results:
                ...
                if client_elapsed_time is not None:
                    ...
                    with open("logs_dinamico/response_times_soglia.csv", "a", newline="") as csvfile:
        ...
        if server_round > 2:
            # Calcola la soglia dinamica basata su delta_loss
            delta_losses = [
                fit_res.metrics.get("delta_loss", 0.0) 
                for _, fit_res in filtered_results 
                if "delta_loss" in fit_res.metrics
            ]

            if delta_losses:
                avg_delta = np.mean(delta_losses)
                dynamic_threshold =  avg_delta * 0.8
                log(INFO, f"[ROUND {server_round}] Soglia dinamica delta_loss impostata a {dynamic_threshold:.5f} (media: {avg_delta:.5f})")

                for client, fit_res in filtered_results:
                    client_id = client.cid
                    delta_loss = fit_res.metrics.get("delta_loss", 0.0)

                    # Verifica che superi la soglia dinamica
                    if delta_loss < dynamic_threshold:
                        log(WARNING, f"[ROUND {server_round}] Client {client.cid} escluso (delta_loss {delta_loss:.5f} < soglia dinamica {dynamic_threshold:.5f})")
                        continue
                ...
                log(WARNING, f"[ROUND {server_round}] Nessun valore di delta_loss disponibile per il calcolo della soglia dinamica.")
                return None, {}

        else:
            # Round iniziali: nessun filtro sulla loss, solo norm opzionalmente
            client_utili = filtered_results


        if not client_utili:
            log(WARNING, f"Nessun client idoneo per l'aggregazione dopo tutti i filtri nel round {server_round}.")
            return None, {}

        if self.inplace:
            #aggregated_ndarrays = aggregate_inplace(filtered_results)
            #aggregated_ndarrays = aggregate_inplace(filtered_results_by_norm)
            aggregated_ndarrays = aggregate_inplace(client_utili)
        else:
            weights_results = [
                (parameters_to_ndarrays(fit_res.parameters), fit_res.num_examples)
                for _, fit_res in client_utili
            ]
        ...
        if self.fit_metrics_aggregation_fn:
            fit_metrics = [(res.num_examples, res.metrics) for _, res in client_utili]
