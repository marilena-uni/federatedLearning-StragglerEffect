...
class FedAvg(Strategy):
    ...
    def aggregate_fit(
        ...
        Path("logs_timeout").mkdir(exist_ok=True)
        #selected_clients = len(results) + len(failures)
        ...
        if server_round <= 2:
            ...
            for client, fit_res in results:
                ...
                if client_elapsed_time is not None:
                    ...
                    with open("logs_timeout/response_times_soglia.csv", "a", newline="") as csvfile:
            ...
            for client, fit_res in results:
                ...
                if client_elapsed_time is not None:
                    ...
                    if client_elapsed_time <= MAX_TIME:
                        ...
                        #participation_rate = aggregated_clients / selected_clients if selected_clients > 0 else 0.0

                        with open("logs_timeout/response_times_soglia.csv", "a", newline="") as csvfile:
                            writer = csv.writer(csvfile)
                            writer.writerow([server_round, average_time_filtered])

                        with open("logs_timeout/numclient_round.csv", "a", newline="") as csvfile:
                                writer = csv.writer(csvfile)
                                writer.writerow([server_round, average_time_filtered, aggregated_clients, participation_rate ])
                    else:
...
This implementation includes a dynamic timeout to handle slow clients (stragglers).
Paper: arxiv.org/abs/1602.05629
"""
...
class FedAvg(Strategy):
    """Federated Averaging strategy with dynamic timeout.
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.response_times_history: List[float] = []
        self.client_start_times: dict[str, float] = {}
        self.delayed_updates: List[Tuple[ClientProxy, FitRes]] = []
    ...
    def configure_fit(
        ...
        # Record start time for each client for timeout calculation
        now = time.time()
        for client in clients:
            self.client_start_times[client.cid] = now
    ...
    def aggregate_fit(
        ...
        """Aggregate model parameters and metrics."""
        if not results:
            log(INFO, "No results received.")
            return None, {}
        if not self.accept_failures and failures:
        ...
        current_round_times_filtered = []
        aggregated_clients=0
        # Logic for dynamic timeout
        if server_round > 2 and self.response_times_history:
            # Calculate dynamic timeout based on historical data
            average_time = sum(self.response_times_history) / len(self.response_times_history)
            MAX_TIME = max(2.0, min(60.0, average_time * 1.2))
            log(INFO, f"[ROUND {server_round}] Dynamic timeout set to {MAX_TIME:.2f}s (based on historical average).")
            selected_clients=len(results)+len(failures)
            # Filter results based on timeout
            for client, fit_res in results:
                client_elapsed_time = fit_res.metrics.get("elapsed_time")
                if client_elapsed_time is not None:
                    if client_elapsed_time <= MAX_TIME:
                        ...
                        log(WARNING, f"[ROUND {server_round}] Client {client.cid} EXCLUDED. Time of {client_elapsed_time:.2f}s exceeded limit of {MAX_TIME:.2f}s.")
                else:
                    log(INFO, f"[ROUND {server_round}] Client {client.cid} did not send 'elapsed_time' metric.")
        else:
            # In initial rounds (or if no history), accept all results
            filtered_results = results
        ...
        # Update historical response times with data from the current successful round
        self.response_times_history.extend(current_round_times)

        if not filtered_results:
            log(WARNING, f"No suitable clients for aggregation in round {server_round}.")
            return None, {}

        # Aggregate the parameters of the successful clients
        if self.inplace:
            aggregated_ndarrays = aggregate_inplace(filtered_results)
        else:
            weights_results = [
